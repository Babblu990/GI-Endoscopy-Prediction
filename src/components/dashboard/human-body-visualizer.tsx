...
export function HumanBodyVisualizer({ isDetected, className }: HumanBodyVisualizerProps) {
  return (
    <div className={cn("relative w-full h-full flex items-center justify-center p-8", className)}>
      ...
      >
        ...
        {/* GI Tract Highlight Region */}
        <g className={cn("transition-all duration-700", isDetected ? "opacity-100" : "opacity-20")}>
          {/* Esophagus/Stomach area */}
          <path
            d="M95 120C95 120 85 140 85 160C85 180 100 190 115 180C130 170 120 140 120 140"
            className={cn("stroke-primary transition-colors", isDetected && "stroke-accent fill-accent/20")}
            strokeWidth="8"
            strokeLinecap="round"
          />
          {/* Intestinal area */}
          ...
          {isDetected && (
            <circle cx="100" cy="200" r="30" className="fill-accent/30 animate-ping" />
          )}
        </g>
      </svg>
